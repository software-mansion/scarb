use crate::plugin::{ProcMacroResult, TokenStream};
use std::ffi::CString;
...
impl StableTokenStream {
    ...
    ///
    /// # Safety
    pub unsafe fn to_token_stream(self) -> TokenStream {
        TokenStream::from(self.to_string())
    }
    ...
    pub unsafe fn from_token_stream(token_stream: TokenStream) -> Self {
        let cstr = CString::new(token_stream.to_string()).unwrap();
...
impl StableProcMacroResult {
    ...
    ///
    /// # Safety
    pub unsafe fn to_proc_macro_result(self) -> ProcMacroResult {
        match self {
            Self::Leave => ProcMacroResult::Leave,
            Self::Remove => ProcMacroResult::Remove,
            Self::Replace(token_stream) => ProcMacroResult::Replace(token_stream.to_token_stream()),
        }
    }
