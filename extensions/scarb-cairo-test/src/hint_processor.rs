...
use cairo_lang_runner::casm_run::{
    MemBuffer, StarknetHintProcessor, cell_ref_to_relocatable, extract_relocatable, vm_get_range,
};
use cairo_lang_runner::{
    CairoHintProcessor, StarknetExecutionResources, StarknetState, insert_value_to_cellref,
};
...
impl<'a> HintProcessorLogic for TestHintProcessor<'a> {
    fn execute_hint(
        &mut self,
...
impl<'a> ResourceTracker for TestHintProcessor<'a> {
...
impl StarknetHintProcessor for TestHintProcessor<'_> {
    fn take_starknet_state(&mut self) -> StarknetState {
        self.cairo_hint_processor.take_starknet_state()
    }

    fn take_syscalls_used_resources(&mut self) -> StarknetExecutionResources {
        self.cairo_hint_processor.take_syscalls_used_resources()
    }
}
