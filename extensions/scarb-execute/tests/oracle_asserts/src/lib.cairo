...
/// Prints a `Span<felt252>` by first trying to deserialize it as `Result<T, (felt252, ByteArray)>`.
...
pub fn print<T, +Serde<T>, +Debug<T>, +Drop<T>>(value: Span<felt252>) -> Span<felt252> {
    ...
    match Serde::<Result<T, (felt252, ByteArray)>>::deserialize(ref value_copy) {
