...
impl<'db> StructSignatureBuilder<'db> {
    ...
    fn new(db: &'db ScarbDocDatabase, id: StructId<'db>, include_private_items: bool) -> Self {
        let members = db
            .struct_members(id)
            .unwrap_or_else(|_| panic!("Failed to get members of struct: {}", id.full_path(db)));
        let signature_data = StructId::retrieve_signature_data(db, id).unwrap_or_else(|_| {
            panic!(
        ...
        if let Some(attributes) = signature_data.attributes {
            let attributes_syntax =
                get_struct_attributes_syntax(attributes, db).unwrap_or_else(|_| {
                    panic!(
        ...
        if let Some(generic_params) = signature_data.generic_params {
            let (stx, _location_links) =
                get_generic_params(generic_params, db).unwrap_or_else(|_| {
                    panic!(
