...
- The [profile.*] section in manifest file is only recognized in the root manifest, and ignored in member manifests.

In a manifest file, the `[workspace]` table supports the following sections:

## `[workspace]`

To create a workspace, you add the `[workspace]` table to a Scarb.toml manifest file:
...
If the `[workspace]` section is added to a Scarb.toml that already defines a `[package]`,
...
Note: paths used to declare path dependencies are relative to workspace root.
...
This functionality is based on [Cargo workspaces](https://doc.rust-lang.org/cargo/reference/workspaces.html)
and both the design and implementation has been heavily influenced by the approach suggested by the Cargo team.
