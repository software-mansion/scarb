...
[`starknet`](/docs/starknet/starknet-package) package.
...
For example, in order to tell `scarb test` to use [Protostar](https://docs.swmansion.com/protostar/) for testing in
...
Do not forget to properly set up Protostar in your project beforehand.
Consult relevant [documentation](https://docs.swmansion.com/protostar/docs/cairo-1/project-initialization) for more
information.
...
For example, to make `scarb test` run `scarb cairo-test` and then Protostar in a sequence, type the following:

```toml filename="Scarb.toml"
[scripts]
test = "scarb cairo-test && protostar test"
```
