...
pub fn quote(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    let mut output_token_stream = rust_quote! {
      let mut quote_macro_result = ::cairo_lang_macro_v2::TokenStream::empty();
    };
        ...
        match quote_token {
            QuoteToken::Content(content) => {
                output_token_stream.extend(rust_quote! {
                  quote_macro_result.push_token(::cairo_lang_macro_v2::TokenTree::Ident(::cairo_lang_macro_v2::Token::new(::std::string::ToString::to_string(#content), ::cairo_lang_macro_v2::TextSpan::call_site())));
                });
            }
            QuoteToken::Var(ident) => {
                output_token_stream.extend(rust_quote! {
                  quote_macro_result.extend(::cairo_lang_macro_v2::TokenStream::from_primitive_token_stream(::cairo_lang_primitive_token::ToPrimitiveTokenStream::to_primitive_token_stream(&#ident)).into_iter());
                });
            }
            QuoteToken::Whitespace => output_token_stream.extend(rust_quote! {
              quote_macro_result.push_token(::cairo_lang_macro_v2::TokenTree::Ident(::cairo_lang_macro_v2::Token::new(" ".to_string(), ::cairo_lang_macro_v2::TextSpan::call_site())));
            }),
        }
    }
