...
fn emits_manifest_diagnostic_data_for_dependency_git_ref_without_git() {
    for (name, dependency, error_code, field, fields) in [
        (
...
fn profile_can_override_cairo_section() {
    ...
    assert_eq!(
        ...
        json!({
            ...
            "unsafe_panic": false,
            "incremental": true,
            "compiler_optimizations": {"Enabled": {"inlining_strategy": "default"}}
        })
    );
}
...
fn can_specify_inlining_strategy_by_weight() {
    ...
    assert_eq!(
        ...
        json!({
            ...
            "unsafe_panic": false,
            "incremental": true,
            "compiler_optimizations": {"Enabled": {"inlining_strategy": 12}}
        })
    );
}
