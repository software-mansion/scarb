...
// NOTE: It relies on the implicit contract: CompilationUnitComponentId.repr == PackageId.to_serialized_string().
//
...
pub struct WorkspaceProcMacros {
    ...
    /// **all macros that the package may depend on in all compilation units it appears in**.
    ...
    pub fn collect(
        ...
        let macros_for_packages = macros_for_components
            ...
            .map(|(package_id, macro_instances)| {
                let deduplicated_instances = macro_instances.into_iter().unique().collect();
