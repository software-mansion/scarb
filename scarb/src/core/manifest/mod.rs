...
pub struct FeaturesDefinition(BTreeMap<FeatureName, Vec<FeatureEnabled>>);

impl FeaturesDefinition {
    pub fn try_new(features: BTreeMap<FeatureName, Vec<FeatureEnabled>>) -> Result<Self> {
        Self::validate(&features)?;
        Ok(Self(features))
    }

    fn validate(features: &BTreeMap<FeatureName, Vec<FeatureEnabled>>) -> Result<()> {
    ...
    pub fn all(&self) -> Keys<'_, FeatureName, Vec<FeatureEnabled>> {
        self.0.keys()
    }

    pub fn get(&self, feature: &FeatureName) -> Option<&Vec<FeatureEnabled>> {
        self.0.get(feature)
    }
    ...
    pub fn default_features(&self) -> Vec<FeatureEnabled> {
    ...
    pub fn iter(&self) -> impl Iterator<Item = (&FeatureName, &Vec<FeatureEnabled>)> {
        self.0.iter()
    }
}
