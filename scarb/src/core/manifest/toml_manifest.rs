...
pub struct TomlCairo {
    ...
    /// Used by [cairo-debugger](https://github.com/software-mansion-labs/cairo-debugger).
    /// This feature is unstable and is subject to change.
    pub add_types_debug_info: Option<bool>,
...
impl TomlManifest {
    ...
    pub fn collect_profiles(&self) -> Result<Vec<Profile>, ManifestSemanticError> {
        ...
        for name in toml_profiles.keys() {
            let profile = Profile::try_new(name.clone())
                .map_err(|err| ProfileNameInvalid::new(name.clone(), err))?;
    ...
    fn collect_profile_definition(&self, profile: Profile) -> Result<TomlProfile> {
        ...
        Self::validate_profile_inheritance(&profile, &parent_profile)?;
    ...
    fn verify_toml_cairo_section(
    ...
    ) -> std::result::Result<(), ManifestSemanticError> {
        ...
        {
            return Err(CairoInliningStrategyConflict::new(profile.as_str()).into());
        }

        Ok(())
    }

    fn validate_profile_inheritance(
        profile: &Profile,
        parent_profile: &Profile,
    ) -> std::result::Result<(), ManifestSemanticError> {
        if parent_profile.is_custom() {
            return Err(
                ProfileInheritanceInvalid::new(profile.as_str(), parent_profile.as_str()).into(),
            );
        }
    ...
    fn collect_compiler_config(profile_definition: TomlProfile) -> ManifestCompilerConfig {
        ...
        if let Some(cairo) = profile_definition.cairo {
            ...
            if let Some(add_types_debug_info) = cairo.add_types_debug_info {
                compiler_config.add_types_debug_info = add_types_debug_info;
            }
