...
pub struct TomlManifest {
    ...
    /// The `[dev-dependencies]` table for dependencies used only in tests and examples.
    ...
    /// grouped by target kind (e.g. `lib`, `executable`, `cairo-plugin`, `test`).
    #[schemars(
    ...
    /// The `[scripts]` table with custom commands you can run via `scarb run -s <name>`.
    ///
    ...
    /// The `[patch.<registry>]` tables to override dependency sources.
    ///
...
pub struct TomlWorkspace {
    ...
    /// Shared workspace `[workspace.dev-dependencies]` available to members.
    ...
    /// explicitly configured on the target.
...
pub struct TomlPackage {
    ...
    /// Declares a re-export of a package dependency.
...
pub struct TomlTarget<P> {
    ...
    /// Path to the source root of this target (defaults depend on target kind).
    #[schemars(with = "Option<String>")]
...
pub struct TomlCairoPluginTargetParams {
    /// If `true`, mark this plugin as builtin so it can be re-exported by dependents.
