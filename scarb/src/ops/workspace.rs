...
fn read_workspace_root<'c>(
    ...
    let with_source = |err| {
        anyhow::Error::new(ManifestErrorWithSource::new(
            manifest_path,
            &manifest_source,
            err,
        ))
    ...
    let root_package = if toml_manifest.is_package() {
        ...
        validate_virtual_manifest(&toml_manifest).map_err(with_source)?;
        None
    };
    ...
    if let Some(workspace) = toml_workspace {
        ...
        let mut packages = workspace
            ...
            .map(AsRef::as_ref)
            .map(|package_path| {
                let package_manifest_source = fs::read_to_string(package_path)
                    .with_context(|| format!("failed to read manifest at: {package_path}"))?;
                let package_manifest = TomlManifest::read_from_str(&package_manifest_source)
        ...
        let package = root_package.ok_or_else(|| {
            ManifestErrorWithSource::new(
                manifest_path,
                &manifest_source,
                anyhow!("the [package] section is missing"),
            )
        })?;
