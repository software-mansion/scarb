...
impl Handler for DefinedMacros {
    fn handle(
        ...
        let manifest_path = convert_manifest_path_to_utf8(&workspace.manifest_path)?;
...
/// Converts the given path to UTF-8.
fn convert_manifest_path_to_utf8(manifest_path: &Path) -> Result<&Utf8Path> {
    let os_string = manifest_path.as_os_str();
    let regular_string = manifest_path
        .to_str()
        .with_context(|| format!("manifest path {os_string:?} contains non-UTF-8 characters"))?;
    Ok(Utf8Path::new(regular_string))
}
