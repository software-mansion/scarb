...
use crate::core::{
    DependencyVersionReq, ManifestDependency, PackageName, SourceId, Target, TargetKind,
};
...
impl WorkspaceResolve {
    ...
    pub fn solution_with_target_kind(
        ...
        assert!(self.packages.contains_key(&root_package));
        self.resolve
            .solution_with_target_kind(root_package, target_kind)
            .iter()
            .map(|id| self.packages[id].clone())
            .collect_vec()
    }
    ...
    ws.config().tokio_handle().block_on(
        async {
            ...
            patch_map.insert(
                ...
                [
                    ManifestDependency::for_all_targets(
                        PackageName::CORE,
                        version_req.clone(),
                        SourceId::for_std(),
                    ),
...
fn generate_cairo_compilation_units(
    ...
    member
        ...
        .map(|member_target| {
            ...
            let test_package_id = member.id.test_package_id();
...
impl<'a> PackageSolutionCollector<'a> {
    ...
    fn pull_from_graph(
        ...
        let mut classes = self
            .resolve
            .solution_with_target_kind(self.member.id, target_kind)
            .into_iter()
...
fn build_cfg_set(target: &Target) -> CfgSet {
    let mut cfg = vec![Cfg::kv("target", target.kind.clone())];
    if target.is_test() {
        cfg.push(Cfg::name("test"));
    }
    CfgSet::from_iter(cfg)
}
